...
ScanLint reads metadata. These are the places where that is not the whole story, stated here rather than discovered later.

**It cannot see text printed into the image.** Some scans have the patient's name burned into the pixels by the scanner or a later tool. ScanLint reports what the file's Burned In Annotation field claims, and that field is unreliable in both directions — files that declare `NO` sometimes have text, and files with no text sometimes declare nothing at all. Look at the image yourself.

**It reads one file at a time.** A study is usually hundreds of slices, and a field can be clean in one and identifying in another. Checking a folder is planned; today, one file is one answer.

**Private tags are reported, not interpreted.** Odd-numbered groups hold manufacturer-defined data whose meaning is not in any public standard. ScanLint tells you they are there and shows their contents where it can. What is inside them is between you and the vendor's conformance statement. In files written with implicit VR, their contents cannot be read at all — the file does not declare the data type and no dictionary supplies it.
...
The sample file is synthetic. It was generated by `scripts/make-sample-study.py` with invented patient details and a noise image, and it contains no real patient data. Every test in this repository runs against it and against two variants of it in other transfer syntaxes.
